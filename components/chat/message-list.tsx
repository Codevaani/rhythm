...
export function MessageList({ messages, isStreaming, error, onRetry, isLoaded }: MessageListProps) {
  ...
  return (
    <div
      ...
      className="absolute inset-0 overflow-y-auto pt-16 pb-32 space-y-4 border-none"
      role="log"
    ...
    >
      <div className="max-w-3xl mx-auto px-4 sm:px-6 w-full space-y-4">
        ...
        {messages.length === 0 && !error && !isStreaming && (
          <div className="flex flex-col items-center justify-center min-h-[60vh] text-center text-stone-400">
          ...
          <a 
            href="https://instagram.com/rhythm.j_official" 
            target="_blank" 
            rel="noopener noreferrer"
            className={`text-sm mt-1 text-emerald-600 hover:text-emerald-700 font-medium transition-colors ${hasAnimated ? "text-blur-intro-delay" : ""}`}
          >
            @rhythm.j_official
          </a>
          <p className={`text-xs mt-2 text-gray-400 ${hasAnimated ? "text-blur-intro-delay" : ""}`}>
